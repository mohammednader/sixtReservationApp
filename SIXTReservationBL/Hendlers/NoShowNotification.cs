...
{
    public class NoShowNotification : SixtReservationHandler<NotificationVM>, IBuilder
    {
        public NoShowNotification(int currentStep, long reservationNo, int reservationStatus, IUnitOfWork _UnitOfWork) : base(_UnitOfWork)
        {
                ...
                if (JobTitles?.Count > 0)
                {
                    ...
                    for (int i = 0; i < ToUsers.Count; i++)
                    {
                        var notify = new Notification
                        {
                            ...
                            IsDeleted = false,
                            GroupId = (int)NotificationGroupType.NeedAssignmentNotificationNoShow
                        };
