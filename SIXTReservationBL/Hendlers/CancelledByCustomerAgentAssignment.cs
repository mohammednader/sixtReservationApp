...
{
    public class CancelledByCustomerAgentAssignment : SixtReservationHandler<AgentAssignmentVM>, IBuilder
    {

        public CancelledByCustomerAgentAssignment(int currentStep, long reservationNo, int reservationStatus, int savedLoggedId,IUnitOfWork unitOfWork) : base(unitOfWork)
        {
        ...
        public override bool PerformAction(AgentAssignmentVM request)
        {
            ...
            var LastNotifications = unitOfWork.NotificationBL.Find(n => n.ReservationNo == ReservationNo &&
                                                                           (n.GroupId == (int)NotificationGroupType.NeedAssignmentNotification || n.GroupId == (int)NotificationGroupType.NoAssignmentNotification))
                                                                                .ToList();
            ...
            var NotifyObject = new Notification
            {
                ...
                UrlNotification = "/Reservation/CByCustomerSteps/" + ReservationNo,
                IsDeleted = false,
                GroupId = (int)NotificationGroupType.AssignedToMe,
                ReservationLogId = SavedLoggedId,
