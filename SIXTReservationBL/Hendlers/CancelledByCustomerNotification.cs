...
{
    public class CancelledByCustomerNotification : SixtReservationHandler<NotificationVM>, IBuilder
    {
        public CancelledByCustomerNotification(int currentStep, long reservationNo, int reservationStatus, IUnitOfWork _UnitOfWork) : base(_UnitOfWork)
        {
            ...
            if (JobTitles?.Count > 0)
            {
                ...
                for (int i = 0; i < ToUsers.Count; i++)
                {
                    var notify = new Notification
                    {
                        ...
                        IsDeleted = false,
                        GroupId= (int)NotificationGroupType.NeedAssignmentNotification
                    };
